...
void addItem(struct ShoppingList* list)
{
	...
	int i = list->length;
	//EXAMINATION
	if(list->RecipeName[0] == '\0')
	{
		printf("Enter recipe name: ");
		fgets(list->RecipeName, MAXNAME, stdin);
		list->RecipeName[strlen(list->RecipeName) - 1] = '\0';
		printf("Enter amount of portions: ");
		scanf("%d", &list->Portioner);
		getchar();
	}
...
void printList(struct ShoppingList* list)
{
	...
	printf("%s\n", list->RecipeName); //EXAMINATION
	printf("Portions: %d\n\n", list->Portioner); //EXAMINATION
...
//EXAMINATION
void ChangeRecipeName(struct ShoppingList* list) {
	
	int ChangeName = 0;

	if (list->RecipeName[0] == '\0')
	{
		printf("You don't have a recipe list!");
		return;
	}

	else
	{
		printf("Current recipe name: %s\n", list->RecipeName);
		printf("Do you want to change your recipe name? 1 = YES, 0 = NO: ");
		do
		{
			scanf("%d", &ChangeName);
		} while (ChangeName < 0 || ChangeName > 1);
...
//EXAMINATION
void scaleRecipe(struct ShoppingList* list) {
	getchar();
	...
	{
		printf("Enter how many portions you want to scale up to: ");
		scanf("%d", &scaledPortion);
		changeFactor = scaledPortion / list->Portioner;

		printf("%s\n", list->RecipeName);
		printf("Portions: %d\n\n", scaledPortion);
		for (int i = 0; i < list->length; i++) {
			printf("%d.  %-20s\t %.2f\t %s\n", i + 1, list->itemList[i].productName,
				list->itemList[i].amount * changeFactor, list->itemList[i].unit);
...
void loadList(struct ShoppingList* list)
{
	...
	int trashTemp[5], tempLength, i, tempPortions;
	char fileName[50], trashRecipe[MAXNAME];
	...
	if (list->length == 0)
	{
		...
		fgets(list->RecipeName, MAXNAME, fp); //EXAMINATION
		list->RecipeName[strlen(list->RecipeName) - 1] = '\0'; //EXAMINATION
		fscanf(fp, "%d", &list->Portioner); //EXAMINATION
		fgetc(fp); //EXAMINATION
		for (int i = 0; i < list->length; i++)
		{
	...
	else
	{
		...
		fgets(trashRecipe, MAXNAME, fp); //EXAMINATION
		fscanf(fp, "%d", &tempPortions); //EXAMINATION
		fgetc(fp);
